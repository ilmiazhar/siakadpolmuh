...
class Web_App_Model extends CI_Model {
	...
	{
		$u = mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $usr);
		$p = md5(mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $psw));
	...
	function getSemuaJadwal()
	{
		...
		LEFT JOIN 
		(select k.kd_jadwal,l.kelas_program,l.kd_mk,k.nim from tbl_krs k left join tbl_jadwal l on k.kd_jadwal=l.kd_jadwal)  as detail
	...
	{
		$q= $this->db->query("select nama_mahasiswa, jurusan,tbl_krs.nim,status from tbl_krs left join tbl_perwalian_header on tbl_krs.nim=tbl_perwalian_header.nim left join tbl_mahasiswa on tbl_krs.nim=tbl_mahasiswa.nim where kd_jadwal='".$kd_jadwal."' and status='".$status."'");
		return $q;
	}
	...
	function getDetailKrsPersetujuan($nim,$program) {
		...
		(select det.kd_jadwal,x.nim,det.nama_dosen,det.kd_dosen,det.nama_mk,det.jum_sks,det.kd_mk,det.semester,det.kelas,det.jadwal,det.kapasitas,det.Peserta,det.CalonPeserta from tbl_krs x left join
		...
		(select o.nim,p.kd_jadwal,
				SUM(CASE status WHEN 1 THEN 1 ELSE 0 END ) AS Peserta, 
				SUM(CASE status WHEN 0 THEN 1 ELSE 0 END ) AS CalonPeserta from tbl_perwalian_header o left join tbl_krs p on o.nim=p.nim group by p.kd_jadwal) d on k.kd_jadwal=d.kd_jadwal
		) det
	...
	function getMahasiswaBimbingan($kd_dosen) {
		...
		left join 
		(select k.nim,k.kd_jadwal,SUM(l.jum_sks) as j_sks from tbl_krs k left join 
	...
	public function getIpkmhs($nim)
	{
		$query = $this->db->query("select round(SUM((tbl_bobot.bobot * tbl_mk.jum_sks))/ SUM(tbl_mk.jum_sks),2) as IPKM FROM tbl_nilai left join (tbl_mk,tbl_bobot) ON tbl_nilai.kd_mk=tbl_mk.kd_mk and tbl_bobot.nilai=tbl_nilai.grade WHERE tbl_nilai.nim='".$nim."'");
		$ipkm=0.0;
		foreach($query->result() as $value)
		{
			$ipkm = $value->IPKM;	
	...
	public function getDetailKrs($nim)
	{
		...
		left join 
		(select det.kelas,
		det.semester,det.kapasitas,det.kd_mk,det.jum_sks,det.kd_jadwal,det.nama_mk,det.nama_dosen,det.jadwal,k.nim from tbl_krs k 
		left join 
	...
	function deleteKrs($nim, $smt)
	{
		...
		$this->db->delete('tbl_krs',$tempwhere_detail);
